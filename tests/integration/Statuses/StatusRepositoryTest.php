...
class StatusRepositoryTest extends \Codeception\TestCase\Test
{
    ...
    public function it_gets_all_statuses_for_a_user()
    {
        ...
        $users = TestDummy::times(2)->create('SocialBucket\Users\User');
        // And statuses for both of them
        $statuses = TestDummy::times(2)->create('SocialBucket\Statuses\Status', ['user_id' => $users[0]->id]);
        TestDummy::times(2)->create('SocialBucket\Statuses\Status', ['user_id' => $users[1]->id]);
    ...
    public function it_saves_a_status_for_a_user()
    {
        ...
        $status = TestDummy::build('SocialBucket\Statuses\Status', [
            'user_id' => null,
            'body' => 'My status'
        ]);
        // And an existing user
        $user = TestDummy::create('SocialBucket\Users\User');
